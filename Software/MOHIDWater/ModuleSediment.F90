...
Module ModuleSediment
    ...
    private:: ReadInitialField3DR4
    private:: ReadInitialField3DR8
    private:: ReadInitialField2DR4
    private:: ReadInitialField2DR8
    interface  ReadInitialField
        module procedure ReadInitialField3DR4
        module procedure ReadInitialField3DR8       
        module procedure ReadInitialField2DR4
        module procedure ReadInitialField2DR8
    end interface  ReadInitialField
...
cd0 :   if (ready_ .EQ. OFF_ERR_) then
    ...
    subroutine ConstructGlobalParameters
        ...
        if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGlobalParameters - ModuleSediment - ERR90'
         ...
         call GetData(Me%Evolution%Geometry,                                             &
                     Me%ObjEnterData,iflag,                                              &
                     SearchType   = FromFile,                                            &
    ...
    subroutine AllocateVariables
        ...
        allocate(Me%BedloadU(ILB:IUB, JLB:JUB))
        Me%BedloadU(:,:) = null_real
        
        allocate(Me%BedloadV(ILB:IUB, JLB:JUB))
        Me%BedloadV(:,:) = null_real
        
        allocate(Me%AuxBedloadX(ILB:IUB, JLB:JUB))
        Me%AuxBedloadX(:,:) = null_real

        allocate(Me%AuxBedloadY(ILB:IUB, JLB:JUB))
        Me%AuxBedloadY(:,:) = null_real
        ...
        if (Me%Residual%ON) then
            ...
            if (Me%Evolution%Old) then
                ...
                if (Me%Residual%LastRun) then
                    ...
                    call ReadInitialField(FieldName = "Bedload U", Field2D = Me%Residual%BedloadU)
                
                    call ReadInitialField(FieldName = "Bedload V", Field2D = Me%Residual%BedloadV)
                endif
...
i1:     if (Me%Boxes%Yes) then
            ...
            allocate(Me%Boxes%BedloadU(ILB:IUB, JLB:JUB))
            Me%Boxes%BedloadU(:,:) = 0.

            allocate(Me%Boxes%BedloadV(ILB:IUB, JLB:JUB))
            Me%Boxes%BedloadV(:,:) = 0.
...
cd2 :           if (BlockFound) then
    ...
    subroutine ConstructCohesiveDryDensity(ClientNumber)
        ...
        if (Me%Evolution%Old) then
            ...
            call ReadInitialField(FieldName = "Cohesive_DryDensity", Field3D = Me%CohesiveDryDensity%Field3D)  
    ...
    subroutine ConstructPropertyValue(NewProperty, ExtractType)
        ...
        if (Me%Evolution%Old) then
            ...
            call ReadInitialField(FieldName = trim(NewProperty%ID%Name), Field3D = NewProperty%Field3D)
    ...
    subroutine ReadInitialField2DR4(FieldName, Field2D)

        !Arguments-------------------------------------------------------------
        character (Len = *)                         :: FieldName
        real(4), dimension(:,:), pointer            :: Field2D
...
cd0:    if (EXIST) then
            ...
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField2DR4; ModuleSediment - ERR10.'
            ...
            call GetDDecompParameters(HorizontalGridID = Me%ObjHorizontalGrid,          &
                                  ...
                                  STAT             = STAT_CALL)
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField2DR4; ModuleSediment - ERR20'
            endif
...
ifMS:       if (MasterOrSlave) then
                ...
                call GetDDecompWorkSize2D(HorizontalGridID = Me%ObjHorizontalGrid,      &
                                            ...
                                            STAT             = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) then
                    stop 'ReadInitialField2DR4; ModuleSediment - ERR30'
                endif
            ...
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField2DR4; ModuleSediment - ERR40'
            endif
            ...
            if (STAT_CALL /= SUCCESS_) stop 'ReadInitialField2DR4; ModuleSediment - ERR50'
            ...
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField2DR4; ModuleSediment - ERR60.'
        
        else if(.not. EXIST) then cd0

                stop 'ReadInitialField2DR4; ModuleSediment - ERR70.'
    ...
    end subroutine ReadInitialField2DR4

    !--------------------------------------------------------------------------
    
    

    subroutine ReadInitialField2DR8(FieldName, Field2D)
...
cd0:    if (EXIST) then
            ...
            call ConstructHDF5 (ObjHDF5,                                                 &
                                trim(Me%Files%Initial)//"5", HDF5_READ, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField2DR8; ModuleSediment - ERR10.'
            ...
            call GetDDecompParameters(HorizontalGridID = Me%ObjHorizontalGrid,          &
                                  ...
                                  STAT             = STAT_CALL)
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField2DR8; ModuleSediment - ERR20'
            endif
...
ifMS:       if (MasterOrSlave) then
                ...
                call GetDDecompWorkSize2D(HorizontalGridID = Me%ObjHorizontalGrid,      &
                                            ...
                                            STAT             = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) then
                    stop 'ReadInitialField2DR8; ModuleSediment - ERR30'
                endif
            ...
            call HDF5SetLimits  (ObjHDF5, ILW, IUW, JLW, JUW, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField2DR8; ModuleSediment - ERR40'
            endif
            ...
            if (STAT_CALL /= SUCCESS_) stop 'ReadInitialField2DR8; ModuleSediment - ERR50'
            ...
            call KillHDF5 (ObjHDF5, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField2DR8; ModuleSediment - ERR60.'
        
        else if(.not. EXIST) then cd0

                stop 'ReadInitialField2DR8; ModuleSediment - ERR70.'
    ...
    end subroutine ReadInitialField2DR8

    !--------------------------------------------------------------------------    
    subroutine ReadInitialField3DR4(FieldName, Field3D)

        !Arguments-------------------------------------------------------------
        character (Len = *)                         :: FieldName
        real(4), dimension(:,:,:), pointer          :: Field3D
...
cd0:    if (EXIST) then
            ...
            call ConstructHDF5 (ObjHDF5,                                                 &
                                trim(Me%Files%Initial)//"5", HDF5_READ, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField3DR4; ModuleSediment - ERR10.'
            
            call GetDDecompParameters(HorizontalGridID = Me%ObjHorizontalGrid,          &
                                  MasterOrSlave    = MasterOrSlave,                 &
                                  STAT             = STAT_CALL)
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField3DR4; ModuleSediment - ERR20'
            endif
...
ifMS:       if (MasterOrSlave) then
                ...
                call GetDDecompWorkSize2D(HorizontalGridID = Me%ObjHorizontalGrid,      &
                                            ...
                                            STAT             = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) then
                    stop 'ReadInitialField3DR4; ModuleSediment - ERR30'
                endif
            ...
            call HDF5SetLimits  (ObjHDF5, ILW, IUW, JLW, JUW, KLB, KUB, &
                                 STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField3DR4; ModuleSediment - ERR40.'
            ...
            call HDF5ReadWindow(HDF5ID        = ObjHDF5,                                &
                                ...
                                STAT          = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                  &
                stop 'ReadInitialField3DR4 - ModuleSediment - ERR50.'
            ...
            call KillHDF5 (ObjHDF5, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField3DR4; ModuleSediment - ERR60.'
        
        else if(.not. EXIST) then cd0

                stop 'ReadInitialField3DR4; ModuleSediment - ERR70.'
    ...
    end subroutine ReadInitialField3DR4

    !--------------------------------------------------------------------------

    subroutine ReadInitialField3DR8(FieldName, Field3D)

        !Arguments-------------------------------------------------------------
        character (Len = *)                         :: FieldName
        real(8), dimension(:,:,:), pointer          :: Field3D
...
cd0:    if (EXIST) then
            ...
            call ConstructHDF5 (ObjHDF5,                                                &
                                trim(Me%Files%Initial)//"5", HDF5_READ, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                  &
                stop 'ReadInitialField3DR8; ModuleSediment - ERR10.'
            
            call GetDDecompParameters(HorizontalGridID = Me%ObjHorizontalGrid,          &
                                  MasterOrSlave    = MasterOrSlave,                     &
                                  STAT             = STAT_CALL)
            if (STAT_CALL /= SUCCESS_) then
                stop 'ReadInitialField3DR8; ModuleSediment - ERR20'
            endif
...
ifMS:       if (MasterOrSlave) then
                ...
                call GetDDecompWorkSize2D(HorizontalGridID = Me%ObjHorizontalGrid,      &
                                            ...
                                            STAT             = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) then
                    stop 'ReadInitialField3DR8; ModuleSediment - ERR30'
                endif
            ...
            call HDF5SetLimits  (ObjHDF5, ILW, IUW, JLW, JUW, KLB, KUB, &
                                 STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField3DR8; ModuleSediment - ERR40.'
            ...
            call HDF5ReadWindow(HDF5ID        = ObjHDF5,                                &
                                ...
                                STAT          = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                  &
                stop 'ReadInitialField3DR8 - ModuleSediment - ERR50.'
            ...
            call KillHDF5 (ObjHDF5, STAT = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                                   &
                stop 'ReadInitialField3DR8; ModuleSediment - ERR60.'
        
        else if(.not. EXIST) then cd0

                stop 'ReadInitialField3DR8; ModuleSediment - ERR70.'
    ...
    end subroutine ReadInitialField3DR8
            ...
            if (Me%ExternalVar%OpenPoints3D (i,j,WKUB) == OpenPoint) then
                ...
                if (Me%CohesiveClass%Run) then
                    ...
                    if(Me%GrainRoughness(i,j) .le. 0.) then
                        ...
                        write(*,*) Me%Mass(i,j), Me%DZ(i,j), Me%ExternalVar%DWZ(i,j,WKUB) - Me%MinLayerThickness
                ...
                do n=1,Me%NumberOfClasses
                    ...
                    if(SandClass%Field3D(i,j,WKUB) > 0.) then
                        ...
                        if (Me%WavesOn) then
                            ...
                            SandClass%NDShearStressWaves(i,j) = Me%ExternalVar%TauWave(i,j)/ &
                                                                (Me%DeltaDensity*Gravity*SandClass%D50) &
                    ...
                    if(SandClass%Field3D(i,j,WKUB) > 0.) then
                        ...
                        if (DeltaTau.GT.0.) then
                            ...
                            if (Me%ExternalVar%VelMod(i,j)> 0.) then                            
                                ...
                                SandClass%BedloadU(i, j) = SandClass%Bedload(i, j) * Uaux
                                SandClass%BedloadV(i, j) = SandClass%Bedload(i, j) * Vaux                              
                            endif
            ...
            do j=WJLB, WJUB               
                ...
                if (Me%ExternalVar%OpenPoints3D (i,j,WKUB) == OpenPoint) then
                    ...
                    if(SandClass%Field3D(i,j,WKUB) > 0.) then                   
                        ...
                        !kg/s/m
                        SandClass%BedLoadU(i, j) = SandClass%FluxU(i, j) / Me%ExternalVar%DVY(i, j)
                        SandClass%BedLoadV(i, j) = SandClass%FluxV(i, j) / Me%ExternalVar%DUX(i, j)                        
                        SandClass%BedLoad(i, j) = (SandClass%BedLoadU(i, j)**2 + SandClass%BedLoadV(i, j)**2)**0.5
                        
                        Me%BedloadU(i, j) = Me%BedloadU(i, j) + SandClass%BedLoadU(i, j)
                        Me%BedloadV(i, j) = Me%BedloadV(i, j) + SandClass%BedLoadV(i, j)
                        Me%Bedload(i, j) = Me%Bedload(i, j) + SandClass%Bedload(i, j) 
...
if1:    if(Me%CohesiveClass%Run) then
            ...
            if(CohesiveMass .gt. 0.) then
                ...
                Me%CohesiveClass%Porosity(i,j,k) = (Me%CohesiveClass%Porosity(i,j,k) * &
                                                    Me%CohesiveClass%Mass(i,j,k) +   &
                                                    Me%CohesiveClass%Porosity(i,j,k+1) * &
                                                    Me%CohesiveClass%Mass(i,j,k+1))/    &
                ...
                Me%CohesiveDryDensity%Field3D(i,j,k) = (Me%CohesiveDryDensity%Field3D(i,j,k) * &
                                                        Me%CohesiveClass%Mass(i,j,k) +   &
                                                        Me%CohesiveDryDensity%Field3D(i,j,k+1) * & 
                                                        Me%CohesiveClass%Mass(i,j,k+1))/   &
                                                       (Me%CohesiveClass%Mass(i,j,k) + Me%CohesiveClass%Mass(i,j,k+1))
...
TOut:       if (Actual >= Me%OutPut%OutTime(OutPutNumber)) then
               ...
               call RotateVectorGridToField(HorizontalGridID  = Me%ObjHorizontalGrid, &
                                                VectorInX         = Me%BedloadU,              &
                                                VectorInY         = Me%BedloadV,              &
                                                VectorOutX        = Me%AuxBedloadX,           &
                                                VectorOutY        = Me%AuxBedloadY,           &   
                                                WaterPoints2D     = Me%ExternalVar%WaterPoints2D,       &
                                                RotateX           = .true.,                             &
                                                RotateY           = .true.,                             &
                                                STAT              = STAT_CALL)               
...
do1:            do n=1,Me%NumberOfClasses
                    ...
                    call RotateVectorGridToField(HorizontalGridID  = Me%ObjHorizontalGrid,          &
                                                VectorInX         = Me%SandClass(n)%BedloadU,       &
                                                VectorInY         = Me%SandClass(n)%BedloadV,       &
                                                VectorOutX        = Me%AuxBedloadX,                 &
                                                VectorOutY        = Me%AuxBedloadY,                 &   
                                                WaterPoints2D     = Me%ExternalVar%WaterPoints2D,   &
                                                RotateX           = .true.,                         &
                    ...
                    call HDF5WriteData  (Me%ObjHDF5, "/Results/Classes/Bedload Y/"//trim(Me%SandClass(n)%ID%Name),      &
                                        trim(Me%SandClass(n)%ID%Name),                                                  &
                                         "kg/s/m", Array2D = Me%AuxBedloadY,                                            &
                                         OutputNumber = OutPutNumber, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR220'
                    
                    call HDF5SetLimits  (Me%ObjHDF5, WorkILB, WorkIUB, WorkJLB,                  &
                                     WorkJUB, WorkKLB, WorkKUB, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR230'
                ...
                if (Me%CohesiveClass%Run) then
                    ...
                    call HDF5WriteData   (Me%ObjHDF5, "/Results/Cohesive Sediment",             &
                                            ...
                                            STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_)                                                  &
                        stop 'OutPutSedimentHDF - ModuleSediment - ERR260'
                    ...
                    call HDF5WriteData   (Me%ObjHDF5, "/Results/Cohesive_DryDensity",           &
                                            ...
                                            STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_)                                                  &
                        stop 'OutPutSedimentHDF - ModuleSediment - ERR270'
                ...
                if (Me%Residual%ON) then
                    ...
                    call HDF5SetLimits  (Me%ObjHDF5, WorkILB, WorkIUB, WorkJLB,                  &
                                         WorkJUB, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR310'
                    
                    call HDF5WriteData  (Me%ObjHDF5, "/Residual/DZ_Residual", "DZ_Residual",    &
                                        "m", Array2D =Me%DZ_Residual,                           &
                                        OutputNumber = OutPutNumber, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR320'
                    
                    call RotateVectorGridToField(HorizontalGridID  = Me%ObjHorizontalGrid,  &
                                                VectorInX         = Me%Residual%BedloadU,   &
                                                VectorInY         = Me%Residual%BedloadV,   &
                                                VectorOutX        = Me%AuxBedloadX,         &
                                                VectorOutY        = Me%AuxBedloadY,         &
                                                WaterPoints2D     = Me%ExternalVar%WaterPoints2D,&
                                                RotateX           = .true.,                 &
                                                RotateY           = .true.,                 &
                                                STAT              = STAT_CALL)               
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR330'
                    
                    call HDF5WriteData  (Me%ObjHDF5, "/Residual/Bedload X",             &
                                         "Bedload X",                                   &
                                         "kg/s/m", Array2D = Me%AuxBedloadX,            &
                                         OutputNumber = OutPutNumber, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR340'

                    call HDF5WriteData  (Me%ObjHDF5, "/Residual/Bedload Y",             &
                                         "Bedload Y",                                   &
                                         "kg/s/m", Array2D = Me%AuxBedloadY,            &
                                         OutputNumber = OutPutNumber, STAT = STAT_CALL)
                    if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR350'
                
                endif

                !Writes everything to disk
                call HDF5FlushMemory (Me%ObjHDF5, STAT = STAT_CALL)
                if (STAT_CALL /= SUCCESS_) stop 'OutPutSedimentHDF - ModuleSediment - ERR360'
...
!        if (MonitorPerformance) call StopWatch ("ModuleSediment", "OutPutSedimentHDF")
    ...
    subroutine OutPut_TimeSeries
        ...
        call WriteTimeSerie(Me%ObjTimeSerie,                                    &
                            Data2D_8 =Me%AuxBedloadX, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                              &
            stop 'OutPut_TimeSeries - ModuleSediment - ERR07'
        
        call WriteTimeSerie(Me%ObjTimeSerie,                                    &
                            Data2D_8 =Me%AuxBedloadY, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                              &
...
do1:    do n=1,Me%NumberOfClasses
 ...
 do1:    do n=1,Me%NumberOfClasses
            ...
            do I = ILB, IUB
                ...
                Me%Boxes%BedloadU(i,j) = Me%Boxes%BedloadU(i,j)   * (1.- Me%Porosity(i,j,WKUB)) * Me%Density
                Me%Boxes%BedloadV(i,j) = Me%Boxes%BedloadV(i,j)   * (1.- Me%Porosity(i,j,WKUB)) * Me%Density
            ...
            call RotateVectorGridToField(HorizontalGridID  = Me%ObjHorizontalGrid,         &
                                        VectorInX         = Me%Boxes%BedloadU,              &
                                        VectorInY         = Me%Boxes%BedloadV,              &
                                        VectorOutX        = Me%AuxBedloadX,                   &
                                        VectorOutY        = Me%AuxBedloadY,                   &   
                                        WaterPoints2D     = Me%ExternalVar%WaterPoints2D,  &
                                        RotateX           = .true.,                        &
...
do1 :               do i=1, Me%NumberOfClasses
                ...
                deallocate(Me%FluxV)
                nullify(Me%FluxV)
                deallocate(Me%AuxBedloadX)
                nullify(Me%AuxBedloadX)
                deallocate(Me%AuxBedloadY)
                nullify(Me%AuxBedloadY)
                deallocate(Me%Bedload)
                nullify(Me%Bedload)
                deallocate(Me%BedloadU)
                nullify(Me%BedloadU)
                deallocate(Me%BedloadV)
                nullify(Me%BedloadV)
                deallocate(Me%DM)
                nullify(Me%DM)
                deallocate(Me%Mass)
                ...
                if (Me%Residual%ON) then
                    ...
                    deallocate(Me%Residual%BedloadU)
                ...
                if (Me%Boxes%Yes) then
                    ...
                    deallocate(Me%Boxes%BedloadU)
                    nullify   (Me%Boxes%BedloadU)
                    deallocate(Me%Boxes%BedloadV)
                    nullify   (Me%Boxes%BedloadV)
