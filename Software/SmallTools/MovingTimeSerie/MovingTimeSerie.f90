...
!
! TITLE         : Mohid Model
! PROJECT       : MovingTimeSerie
! PROGRAM       : MainMovingTimeSerie
...
!   Z_COLUMN                : int                   [4]         !Column with coordinates in ZZ axis (depth in meters e.g. surface z = 0 )
...
!   LAYER                   : int                   [1]         !Number of the layer where to get the values
!   VARIABLE_Z              : 0/1                   [0]         !Moving time series with variable depth (e.g. ARGO buoys)
!   <BeginHDF5File>                                                                 
...
program MohidMovingTimeSerie
    ...
    type     T_Grid
        ...
        real, dimension(:,:  ), pointer                     :: WaterPoints2D
    ...
    type T_MovingTimeSerie
        character(PathLength)                               :: MovingTimeSerieFile
        character(PathLength)                               :: DataFile
        integer                                             :: ObjTimeSerie         = 0
        integer                                             :: ObjOutTimeSerie      = 0
        integer                                             :: X_Column
        integer                                             :: Y_Column
        integer                                             :: Z_Column
        integer                                             :: Layer
        logical                                             :: VariableZ            = .true.
        ...
        integer,        dimension(:    ), pointer           :: I, J, K
        ...
        real,           dimension(:,:  ), pointer           :: PreviousField2D
        real,           dimension(:,:,:), pointer           :: PreviousField3D
        real,           dimension(:,:,:), pointer           :: VerticalZ
        real,           dimension(:,:  ), pointer           :: NextField2D
        real,           dimension(:,:,:), pointer           :: NextField3D
        real                                                :: DT
    ...
    end type T_MovingTimeSerie

    type(T_MovingTimeSerie), pointer                        :: Me


    call ConstructMohidMovingTimeSerie
    call ModifyMohidMovingTimeSerie
    call KillMohidMovingTimeSerie

    contains
    
    !--------------------------------------------------------------------------

    subroutine ConstructMohidMovingTimeSerie
        ...
        call StartUpMohid("MohidMovingTimeSerie")
        ...
        if(STAT_CALL .ne. SUCCESS_)                                             &
            stop 'ConstructMohidMovingTimeSerie MohidMovingTimeSerie - ERR01'

        call StartTimeSerie(Me%ObjOutTimeSerie, Me%ObjTime,                     &
                            trim(Me%DataFile),                                  &
                            Me%ParameterList, "srm",                            &
                            STAT = STAT_CALL)
        if(STAT_CALL .ne. SUCCESS_)                                             &
            stop 'ConstructMohidMovingTimeSerie MohidMovingTimeSerie - ERR02'
    ...
    end subroutine ConstructMohidMovingTimeSerie
    ...
    subroutine AllocateFields
        ...
        allocate(Me%Values2D       (Me%Size3D%ILB:Me%Size3D%IUB, &
                                    Me%Size3D%JLB:Me%Size3D%JUB))

        allocate(Me%PreviousField2D(Me%Size3D%ILB:Me%Size3D%IUB, &
                                    Me%Size3D%JLB:Me%Size3D%JUB))

        allocate(Me%NextField2D    (Me%Size3D%ILB:Me%Size3D%IUB, &
                                    Me%Size3D%JLB:Me%Size3D%JUB))

        if(Me%VariableZ)then
    ...
    subroutine ModifyMohidMovingTimeSerie
        ...
        write(*,*)
        write(*,*)'------->   ', trim(Me%MovingTimeSerieFile)
        write(*,*)
        ...
        do iPosition = 1, Me%nPositions
            ...
            Me%Values2D(:,:) = Me%X(iPosition)

            call WriteTimeSerie(Me%ObjOutTimeSerie,                                         &
                                Data2D = Me%Values2D,                                       &
                                I_cell  = OutI, J_cell = OutJ,                              &
                                STAT   = STAT_CALL)

            Me%Values2D(:,:) = Me%Y(iPosition)
            ...
            do while(associated(HDF5File))
                ...
                if(HDF5File%CurrentInstant > 1)then
                    ...
                    select case(ObjParameter%Rank)
                        ...
                        case(2)
                            ...
                            call InterpolateValueInTime(Me%CurrentTime,                                     &
                                                        PreviousTime, Me%PreviousField2D(OutI, OutJ),       &
                                                        NextTime, Me%NextField2D(OutI, OutJ),               &
                                                        Me%Values2D(OutI, OutJ))
                        ...
                        case(3)
                            ...
                            call InterpolateValueInTime(Me%CurrentTime,                                     &
                                                        PreviousTime, Me%PreviousField3D(OutI, OutJ, OutK), &
                                                        NextTime, Me%NextField3D(OutI, OutJ, OutK),         &
                                                        Me%Values3D(OutI, OutJ, OutK))

                            call WriteTimeSerie(Me%ObjOutTimeSerie,                                         &
                                                Data3D = Me%Values3D,                                       &
                                                I_cell  = OutI, J_cell = OutJ,  K_Cell = OutK,              &
                                                STAT   = STAT_CALL)
            ...
            if(STAT_CALL .ne. SUCCESS_)stop 'ModifyMohidMovingTimeSerie - ERR01'
    ...
    end subroutine ModifyMohidMovingTimeSerie
    
    !--------------------------------------------------------------------------

    subroutine KillMohidMovingTimeSerie
        ...
        do while (associated(HDF5File))
            ...
            if(STAT_CALL .ne. SUCCESS_)stop 'KillMohidMovingTimeSerie - ERR01'
        ...
        call KillTimeSerie(Me%ObjOutTimeSerie, STAT = STAT_CALL)
        if(STAT_CALL .ne. SUCCESS_)stop 'KillMohidMovingTimeSerie - ERR02'

        call StopCPUTime

        call ShutdownMohid ("MohidMovingTimeSerie", ElapsedSeconds, TotalCPUTime)

    end subroutine KillMohidMovingTimeSerie
    ...
    subroutine ReadGlobalData
        ...
        call ReadFileName('IN_MODEL', Me%DataFile, "MohidMovingTimeSerie", STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ReadKeywords - MohidMovingTimeSerie - ERR01'

        call ConstructEnterData (Me%ObjEnterData, Me%DataFile, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ReadKeywords - MohidMovingTimeSerie - ERR02'

        !Moving Time Serie File
        call GetData(Me%MovingTimeSerieFile, Me%ObjEnterData, iflag,        &
                     keyword      = 'MOVING_TIMESERIE_FILE',                &
                     SearchType   = FromFile,                               &
                     ClientModule = 'MohidMovingTimeSerie',                 &
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_ .or. iflag == 0)                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR03'
        ...
        call GetData(Me%X_Column, Me%ObjEnterData, iflag,                   &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_ .or. iflag == 0)                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR04'
        ...
        call GetData(Me%Y_Column, Me%ObjEnterData, iflag,                   &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_ .or. iflag == 0)                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR05'


        call GetData(Me%Z_Column, Me%ObjEnterData, iflag,                   &
                     keyword      = 'Z_COLUMN',                             &
                     SearchType   = FromFile,                               &
                     Default      = 4,                                      &
                     ClientModule = 'MohidMovingTimeSerie',                 &
        ...
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR06'
        ...
        call GetData(Me%Use_LatLon, Me%ObjEnterData, iflag,                 &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR07'

        call GetData(Me%VariableZ, Me%ObjEnterData, iflag,                  &
                     keyword      = 'VARIABLE_Z',                           &
                     SearchType   = FromFile,                               &
                     Default      = .false.,                                &
                     ClientModule = 'MohidMovingTimeSerie',                 &
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR080'

        if(.not. Me%VariableZ)then

            call GetData(Me%Layer, Me%ObjEnterData, iflag,                  &
                         keyword      = 'LAYER',                            &
                         SearchType   = FromFile,                           &
                         Default      = 1,                                  &
                         ClientModule = 'MohidMovingTimeSerie',             &
                         STAT         = STAT_CALL)
            if (STAT_CALL /= SUCCESS_)                                      &
                stop 'ReadGlobalData - MohidMovingTimeSerie - ERR081'
        endif
        ...
        call GetData(Me%CorrectTrajectory, Me%ObjEnterData, iflag,          &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR09'
        ...
        call GetData(Me%Correction_Method, Me%ObjEnterData, iflag,          &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR10'
        ...
        call GetData(Me%Tolerance, Me%ObjEnterData, iflag,                  &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR11'
        ...
        call GetData(Me%ToleranceIncrease, Me%ObjEnterData, iflag,          &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR12'
        ...
        call GetData(Me%MaxTolerance, Me%ObjEnterData, iflag,               &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR13'
        ...
        call GetData(Me%MaxNumberOfForecasts, Me%ObjEnterData, iflag,       &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                          &
            stop 'ReadGlobalData - MohidMovingTimeSerie - ERR14'

        call ReadMovingTimeSerieFile
        ...
        if (STAT_CALL /= SUCCESS_) stop 'ReadKeywords - MohidMovingTimeSerie - ERR03'

    end subroutine ReadGlobalData

    !--------------------------------------------------------------------------

    subroutine ReadMovingTimeSerieFile
        ...
        call ConstructEnterData (ObjEnterData, Me%MovingTimeSerieFile, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR01'
            
        call StartTimeSerieInput(Me%ObjTimeSerie,                               &
                                 Me%MovingTimeSerieFile,                        &
                                 STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR02'

        call GetTimeSerieDataMatrix(Me%ObjTimeSerie, FieldData, STAT = STAT_CALL)
                                    
        if (STAT_CALL /= SUCCESS_)stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR03'
        ...
        call GetTimeSerieHeader   (Me%ObjTimeSerie, Me%Header, STAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_)stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR03'


        allocate(Me%X           (1:Me%nPositions))
        allocate(Me%Y           (1:Me%nPositions))
        allocate(Me%Z           (1:Me%nPositions))
        ...
        Me%Z            (1:Me%nPositions) = FieldData(:, Me%Z_Column)
        ...
        do n = 2, Me%nPositions
            ...
            if(CurrentDT<0)then
                ...
                stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR04'
            end if
        ...
        if (STAT_CALL /= SUCCESS_)stop 'ReadMovingTimeSerieFile - MohidMovingTimeSerie - ERR05'


    end subroutine ReadMovingTimeSerieFile
    ...
    subroutine CorrectPositions(CumulativeTime, FieldData)
        ...
        if(NeedsCorrection)then
            ...
            NameLength          = len_trim(Me%MovingTimeSerieFile)
            XYZFileName         = Me%MovingTimeSerieFile(:NameLength-4)//'corrected.xyz'
            LinesFileName       = Me%MovingTimeSerieFile(:NameLength-4)//'corrected.lin'
            CorrectedFileName   = Me%MovingTimeSerieFile(:NameLength-4)//'corrected.srm'

            call UnitsManager(XYZUnit, OPEN_FILE, STAT = STAT_CALL) 
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR01'

            open(XYZUnit, File = trim(XYZFileName), Form='FORMATTED', &
                 status = 'UNKNOWN',IOSTAT = STAT_CALL) 
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR02'

            call UnitsManager(LinesUnit, OPEN_FILE, STAT = STAT_CALL) 
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR03'

            open(LinesUnit, File = trim(LinesFileName), Form='FORMATTED', &
                 status = 'UNKNOWN',IOSTAT = STAT_CALL) 
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR04'
            ...
            write(CorrectedFileUnit, *)
            write(CorrectedFileUnit, '(A)')trim(Me%Header)
            ...
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR05'
        
            call UnitsManager(LinesUnit, CLOSE_FILE, STAT = STAT_CALL) 
            if (STAT_CALL /= SUCCESS_) stop 'CorrectPositions - MohidMovingTimeSerie - ERR06'
            ...
            write(*,*)
            stop 'CorrectPositions - MohidMovingTimeSerie - ERR06'
        end if
    ...
    subroutine WritePositions(CumulativeTime)
        ...
        NameLength    = len_trim(Me%MovingTimeSerieFile)
        XYZFileName   = Me%MovingTimeSerieFile(:NameLength-4)//'.xyz'
        LinesFileName = Me%MovingTimeSerieFile(:NameLength-4)//'.lin'

        call UnitsManager(XYZUnit, OPEN_FILE, STAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR01'

        open(XYZUnit, File = trim(XYZFileName), Form='FORMATTED', &
             status = 'UNKNOWN',IOSTAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR02'

        call UnitsManager(LinesUnit, OPEN_FILE, STAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR03'

        open(LinesUnit, File = trim(LinesFileName), Form='FORMATTED', &
             status = 'UNKNOWN',IOSTAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR04'
        ...
        call UnitsManager(XYZUnit, CLOSE_FILE, STAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR05'
        
        call UnitsManager(LinesUnit, CLOSE_FILE, STAT = STAT_CALL) 
        if (STAT_CALL /= SUCCESS_) stop 'WritePositions - MohidMovingTimeSerie - ERR06'
...
cd2 :           if (BlockFound) then                                                  
                    ...
                    call GetData(NewHDF5File%Name,                                  &
                                 ...
                                 STAT         = STAT_CALL)
                    if (STAT_CALL .NE. SUCCESS_)                                    &
                        stop 'ConstructHDF5File - MohidMovingTimeSerie - ERR00'
                    ...
                    if (STAT_CALL .NE. SUCCESS_)                                    &
                        stop 'ConstructHDF5FilesList - MohidMovingTimeSerie - ERR01'
                ...
                stop 'ConstructHDF5FilesList - MohidMovingTimeSerie - ERR02'
            else cd1
                stop 'ConstructHDF5FilesList - MohidMovingTimeSerie - ERR03'
            end if cd1
        ...
        if (.not. AtLeastOneBlock) then                                            
            ...
            stop 'ConstructHDF5FilesList - MohidMovingTimeSerie - ERR04'
        end if
    ...
    subroutine ConstructGridFromHDF5(ObjHDF5File)
        ...
        if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR01'
            
        do i = 1, nItems

            call GetHDF5GroupID(ObjHDF5File%ObjHDF5, FatherGroupName = "/Grid",     &
                                GroupPosition = i, GroupName =  GroupName,          &
                                Rank = Rank, Dimensions = Dimensions,               &
                                STAT = STAT_CALL)                                
            if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR02'
            ...
            select case(Rank)
                ...
                case(2)
                    ...
                    if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR03'
                ...
                case(3)
                    ...
                    if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR04'
            ...
            select case(GroupName)
                ...
                case ('Bathymetry')
                    ...
                    call HDF5ReadData(ObjHDF5File%ObjHDF5, "/Grid",                 &
                                      ...
                                      STAT      = STAT_CALL)
                    if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR05'
                ...
                case('ConnectionX', 'Longitude')
                    ...
                    if(Me%Use_LatLon)then
                        ...
                        call HDF5ReadData(ObjHDF5File%ObjHDF5, "/Grid",                     &
                                          ...
                                          Array2D   = Me%Grid%XX,                           &
                                          STAT      = STAT_CALL)
                        if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR06'
                        ...
                        call HDF5ReadData(ObjHDF5File%ObjHDF5, "/Grid",                     &
                                          ...
                                          Array2D   = Me%Grid%XX,                           &
                                          STAT      = STAT_CALL)
                        if (STAT_CALL .NE. SUCCESS_) stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR07'
                ...
                case('ConnectionY', 'Latitude')
                    ...
                    if(Me%Use_LatLon)then
                        ...
                        call HDF5ReadData(ObjHDF5File%ObjHDF5, "/Grid",                     &
                                          ...
                                          STAT      = STAT_CALL)
                        if (STAT_CALL .NE. SUCCESS_)                                        &
                            stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR08'
                        ...
                        call HDF5ReadData(ObjHDF5File%ObjHDF5, "/Grid",                     &
                                          ...
                                          STAT      = STAT_CALL)
                        if (STAT_CALL .NE. SUCCESS_)                                        &
                            stop 'ConstructGridFromHDF5 - MohidMovingTimeSerie - ERR09'

                    end if


                case('WaterPoints3D', 'WaterPoints2D')
    ...
    subroutine ReadParameterRanks(ObjHDF5File)
        ...
        do while(associated(ObjParameter))
            ...
            call GetHDF5GroupID(ObjHDF5File%ObjHDF5, ObjParameter%Group,  1,    &
                                ...
                                STAT = STAT_CALL)                                
            if (STAT_CALL .NE. SUCCESS_) stop 'ReadParameterRanks - MohidMovingTimeSerie - ERR01'
...
cd2 :           if (BlockFound) then
                    ...
                    if (STAT_CALL .NE. SUCCESS_)                                &
                    stop 'ReadParameters - MohidMovingTimeSerie - ERR01'
                ...
                write(*,*)  
                write(*,*) 'Error calling ExtractBlockFromBuffer. '
                stop 'ReadParameters - MohidMovingTimeSerie - ERR02'
            else cd1
                stop 'ReadParameters - MohidMovingTimeSerie - ERR03'
            end if cd1
        ...
        if (.not. AtLeastOneBlock) then                                            
            ...
            stop 'ReadParameters - MohidMovingTimeSerie - ERR04'
        end if
    ...
    subroutine ConstructTimeSerieParameters (NewParameter)
        ...
        call GetData(NewParameter%Name,                         &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL .NE. SUCCESS_)                            &
        stop 'ConstructTimeSerieParameters - MohidMovingTimeSerie - ERR01'

        if (.not.CheckPropertyName(NewParameter%Name)) then
            write(*,*)
            write(*,*) 'The property name is not recognised by the model.'
            write(*,*) 'ConstructTimeSerieParameters - MohidMovingTimeSerie - WARN02' 
        end if
        ...
        call GetData(NewParameter%Group,                        &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL .NE. SUCCESS_)                            &
        stop 'ConstructTimeSerieParameters - MohidMovingTimeSerie - ERR03'
    ...
    subroutine ConstructParameterList
        ...
        allocate(Me%ParameterList(1:nParameters+3))

        Me%ParameterList(1) = 'X'
        Me%ParameterList(2) = 'Y'
        Me%ParameterList(3) = 'Z'

        !Fills up ParameterList
        ParameterX   => Me%FirstParameter

        do n = 4, 3+nParameters

            Me%ParameterList(n) = trim(adjustl(ParameterX%Name))
    ...
    subroutine OrganizeHDF5Files
        ...
        do while (associated(HDF5File))
            ...
            if (.not. exist) then
                ...
                stop 'OrganizeHDF5Files - MohidMovingTimeSerie - ERR01'
            endif
            ...
            if (STAT_CALL .NE. SUCCESS_)                                    &
                stop 'OrganizeHDF5Files - MohidMovingTimeSerie - ERR02'
            ...
            if (STAT_CALL .NE. SUCCESS_)                                    & 
                stop 'OrganizeHDF5Files - MohidMovingTimeSerie - ERR03'
            ...
            if(FirstTime > Me%EndTime .or. LastTime < Me%BeginTime)then
                ...
                do i = 1, NumberOfInstants
                    ...
                    if(.not. FoundFirstInstant .and. AuxInstantsArray(i) .ge. Me%BeginTime)then
                        ...
                        HDF5File%FirstSignificantInstant = i

                    end if

                    if(.not. FoundLastInstant  .and. AuxInstantsArray(i) .ge. Me%EndTime)then

                        FoundLastInstant                = .true.

                        HDF5File%LastSignificantInstant = i
    ...
    type(T_Time) function HDF5TimeInstant(Instant, ObjHDF5File)
        ...
        call HDF5ReadData   (HDF5ID         = ObjHDF5File%ObjHDF5,      &
                             ...
                             STAT           = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)                                      &
            stop 'HDF5TimeInstant - MohidMovingTimeSerie - ERR01'
    ...
    subroutine ReadPreviousAndNextField(ObjHDF5File, ObjParameter, CurrentInstant)
        ...
        if(Me%VariableZ)then
            ...
            lower_bound(3)  = Me%Layer
            upper_bound(3)  = Me%Layer
    ...
    subroutine DeallocateFields
        ...
        deallocate(Me%Values2D       )       
        deallocate(Me%PreviousField2D)
        deallocate(Me%NextField2D    )
...
end program MohidMovingTimeSerie
