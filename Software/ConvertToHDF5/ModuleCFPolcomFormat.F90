...
Module ModuleCFPolcomFormat
    ...
    private :: T_CFPolcomFormat
    type       T_CFPolcomFormat
    ...
    end type  T_CFPolcomFormat

    type(T_CFPolcomFormat), pointer                   :: Me
...
!--------------------------------------------------------------------------------
   ...
   contains
    ...
    subroutine ConvertCFPolcomFormat(EnterDataID, STAT)
    ...
    end subroutine ConvertCFPolcomFormat
    ...
    subroutine ReadOptions
        ...
        call GetData(Me%FileName,                                       &
                     ...
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ReadOptions - ModuleCFPolcomFormat - ERR01'

        if (iflag == 0)then
            write(*,*)'Must specify name of file to convert'
            stop 'ReadOptions - ModuleCFPolcomFormat - ERR02'
        end if
        ...
        call GetData(Me%GridFileName,                                   &
                     ...
                     ClientModule = 'ModuleCFPolcomFormat',                  &
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ReadOptions - ModuleCFPolcomFormat - ERR02'
        ...
        call GetData(Me%OutputFileName,                                 &
                     ...
                     ClientModule = 'ModuleCFPolcomFormat',                  &
                     STAT         = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ReadOptions - ModuleCFPolcomFormat - ERR03'
   ...
   !Begin-----------------------------------------------------------------
     ...
     if (.not. exist) then
        ...
        stop 'OpenAndReadCFFile - ModuleCFPolcomFormat - ERR01'
     endif
    ...
    logical function CheckName(CFVar, MohidName)
        ...
        if (trim(CFVar%standardName)/='') then
          ...
          select case(trim(CFVar%standardName))
            ...
            case('surface_net_downward_longwave_flux')

               MohidName = GetPropertyName(NetLongWaveRadiation_)
               CheckName = .true.
...
!               CheckName = .true.
   ...
   !------------------------------------------------------------------------
    ...
    if (.not. Me%Proyected) then
        ...
        if (STAT_CALL /= SUCCESS_) stop 'ConstructGrid - ModuleCFPolcomFormat - ERR02'
    else
        ...
        call ConstructHorizontalGrid(Me%ObjHorizontalGrid, Me%GridFileName, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_) stop 'ConstructGrid - ModuleCFPolcomFormat - ERR04'
...
!                 End write Polcom SZZ data into hdf5 file
    ...
    subroutine KillCFPolcomFormat
        ...
        if (STAT_CALL /= SUCCESS_)stop 'KillCFPolcomFormat - ModuleMM5Format - ERR01'


        call KillHDF5(Me%ObjHDF5, STAT = STAT_CALL)
        if (STAT_CALL /= SUCCESS_)stop 'KillCFPolcomFormat - ModuleMM5Format - ERR02'

        nUsers = DeassociateInstance(mENTERDATA_, Me%ObjEnterData)
        if (nUsers == 0) stop 'KillCFPolcomFormat - ModuleMM5Format - ERR03'
    ...
    end subroutine KillCFPolcomFormat

    !--------------------------------------------------------------------------

end module ModuleCFPolcomFormat
