!Program to generate voronoi volumes in 3D. 
! 
      Module ModuleVoronoi3D

      implicit none

      private
           
      public  :: Voronoi_3D_volume
      private :: regtet_sub
      private :: pwrvtx_sub
      private :: volare_sub
...
!http://xlr8r.info/mPower/pages/regtet.f.html
!http://xlr8r.info/mPower/pages/pwrvtx.f.html
!http://math.nist.gov/~JBernal/JBernal_Sft.html

      subroutine Voronoi_3D_volume (npoints, x,y,z, volume)


        integer, parameter       :: nx = 8
        integer                  :: npoints
        real                     :: x(npoints), y(npoints), z(npoints)
        real                     :: volume(1:npoints)

        real(8), allocatable  :: xx(:), yy(:), zz(:)

        integer   ::  icfig, nvmax ,nfmax, nv, nmax    
        character ::  nparam(6)

        integer   :: ideli,ipnti,iflpi,iarti,irani,ireci,iredi, nw, nt
        integer   :: icon(8,7*nx*npoints), is(nx*npoints)
        integer   :: icon_2(8,7*nx*npoints), is_2(nx*npoints)
        integer   :: nw2,nt2,nq

        integer   :: ifl(7*nx*npoints)
        real(8)   :: xp(7*nx*npoints),yp(7*nx*npoints),zp(7*nx*npoints)
        Real(8)   :: vol(nx*npoints)
        ...
        allocate(xx(nmax),yy(nmax),zz(nmax))
        ...
        call regtet_sub(nparam, nmax,nvmax, nv,xx,yy,zz,icfig,ideli,
     *                ipnti,iflpi,iarti,irani,ireci,iredi,nw,nt,icon,is)
     
!        if (nt.ge.1) then

        call pwrvtx_sub (nmax,nvmax,ideli,ipnti,iflpi,iarti,irani,
     *         ireci,iredi, nw, nt, icfig,icon(1:8,1:nt),is(1:nw),nv,xx,
     *         yy,zz,icon_2,is_2,nw2,nt2,nq,ifl,xp,yp,zp)

        call volare_sub(nmax,nvmax,nfmax,nv,xx,yy,zz,ideli,nw2,nt2,
     *      nq,icon_2(1:8,1:nq),is_2(1:nw2),ifl(1:nq), xp,yp,zp, vol )

        volume(1:npoints) = vol(1:nv)     
...
!c     Disclaimer:
!c
!c     This software was developed at the 
...
!c     copyright protection and is in the public domain. 
!c     This software is
...
!c     software is used.
!c
      subroutine regtet_sub(param, nmax,nvmax,nv, x,y,z, icfig,
...
c
      ...
      !parameter (nvmax= 7*nmax, nhmax=1500)
      
      !integer, parameter :: nmax = nmax
      !integer, parameter :: nvmax = 7*nmax
      
      
      integer nmax, nvmax, nhmax
      parameter (nhmax=1500)
      character param(6)*1 
...
c----------------------------------------------------------------------
c
!      !write information in file in a readable form
!
!      open (23, file = 'exeinfor')
!      write (23,*)' '
!      write (23,*)'Number of vertices   =',nw
!      write (23,*)'Number of tetrahedra =',nt
!      write (23,*)' '
!      if(nt.ne.0)then
!        do 600 i = 1, nt
!           write (23,800) i, (icon(j,i), j=1,8)
! 600    continue
!        do 650 i = 1, nw
!           write (23,850) i, is(i)
! 650    continue
!      else
!        write (23,*)'warning: no real tetrahedra were created.'
!        write (23,*)' '
!      endif
! 800  format ('tetr: ', i4, ' info: ', 8(1x,i4))
! 850  format ('vert: ', i4, ' is: ',i4)
...
cc      write(*,*)' '
cc      write(*,*)'Checking tetrahedralization ...'
c     WRITE(*,*)' '
...
c
      real var(*)
      integer ji(*), klen, iast, k, i, jj, iis, idif
c
...
c
      idif = iis - iast
      if(idif.lt.0) go to 110
      if(idif.eq.0) go to 140
      go to 150
...
!https://people.sc.fsu.edu/~jburkardt/f_src/sphere_voronoi/
!sphere_voronoi.html
...
c     program regtet/ragtat and that the output tetrahedron list
c     produced by regtet/ragtat contains only real tetrahedra (no
c     artificial ones), i. e. logical variable artfcl was set to
c     .false. during the execution of regtet/ragtat. The output
c     tetrahedron list from regtet/ragtat is then used as input below.
c     It is noted that because of degeneracies more than one
c     tetrahedron may correspond to the same Power/Voronoi vertex.
c     In addition, because of degeneracies, Power/Voronoi cells of
c     points may have facets that are not 2-dimensional, and if
c     weights are present there may be Power cells of points that are
c     not 3-dimensional.
c
...
*MAIN
c
      subroutine pwrvtx_sub (nmax,nvmax,ideli,ipnti,iflpi,iarti,irani,
     *                       ireci, iredi, nw, nt, icfig,icon,is,nv,x,
     *                       y,z,icon_2,is_2,nw2,nt2,nq,ifl,xp,yp,zp)
     
     
c
      integer nmax, nvmax
      !parameter (nmax=150000, nvmax= 7*nmax)
c
      double precision x(nmax), y(nmax), z(nmax), w(nmax)
      integer ix(nmax), iy(nmax), iz(nmax), iw(nmax)
      integer ix2(nmax), iy2(nmax), iz2(nmax), iw2(nmax)
      double precision xp(nvmax), yp(nvmax), zp(nvmax)
      integer icon(8,nvmax), ifl(nvmax), is(nmax)
      integer icon_2(8,nvmax), is_2(nmax)
      
      integer nv, nw, nt, nq, naddl, icfig, iwfig,nw2,nt2
...
c
      ...
      integer i, no !,j
c
      integer ng, ifir, isec, ithi, ifou, larg
...
c
!      write(*,*)' '
!      write(*,*)'This program is for computing the Power diagram ',
!     *          'vertices and the'
!      write(*,*)'unbounded Power diagram edges for a set of ',
!     *          'points in 3-d space'
!      write(*,*)'from a Regular tetrahedralization for the set.'
!      write(*,*)' '
!      write(*,*)'If no weights are present the Voronoi vertices ',
!     *          'and the unbounded'
!      write(*,*)'Voronoi edges are computed from a Delaunay ',
!     *          'tetrahedralization for'
!      write(*,*)'the set.'
!      write(*,*)' '
!      write(*,*)'It is assumed that a Regular/Delaunay ',
!     *          'tetrahedralization for the'
!      write(*,*)'set has already been computed using existing program',
!     *          'regtet/ragtat'
!      write(*,*)'and that the output tetrahedron list produced by ',
!     *          'regtet/ragtat'
!      write(*,*)'contains only real tetrahedra (logical variable ',
!     *          'artfcl was set to'
!      write(*,*)'.false. during the execution of regtet/ragtat).'
!      write(*,*)' '
!      write(*,*)'The output tetrahedron list from regtet/ragtat ',
!     *          'is then used as'
!      write(*,*)'input by this program. Note that because of ',
!     *          'degeneracies more than'
!      write(*,*)'one tetrahedron may correspond to the same ',
!     *          'Power/Voronoi vertex.'
!      write(*,*)'In addition, because of degeneracies, Power/Voronoi ',
!     *          'cells of'
!      write(*,*)'points may have facets that are not 2-dimensional, ',
!     *          'and if weights'
!      write(*,*)'are present there may be Power cells of points that ',
!     *          'are not'
!      write(*,*)'3-dimensional.'
!      write(*,*)' '
!      write(*,*)'Arrays xp, yp, zp, icon, ifl will contain output ',
!     *          'information.'
!      write(*,*)'Description of these arrays appears in driver ',
!     *          'routine pwrvtx.'
!      write(*,*)' '
!      write(*,*)'In the current main program arrays ifl, xp, yp, zp ',
!     *          'are saved in'
!      write(*,*)'output file power-vrts, and array icon in output ',
!     *          'file tetra-unbd.'
c
c----------------------------------------------------------------------
c
...
!!!      read (12,80) ideli, ipnti, iflpi, iarti, irani, ireci, iredi
      delaun = .false.
      pntoff = .false.
      flphis = .false.
      artfcl = .false.
...
!!!      if(.not.delaun) then
!!!         read (12,*) nw, nt, icfig, iwfig
!!!      else
!!!         read (12,*) nw, nt, icfig
!!!      endif
      if(nw.gt.nmax .or. nt.gt.nvmax) stop 10
!!!      read (12,90) ((icon(i,j), i = 1, 8), j = 1, nt)
!!!      read (12,95) (is(i), i = 1, nw)
!!!      if(reccor .and. .not.delaun) then
!!!         read (12,*) wlenx, wleny, wlenz, wlenw
!!!         read (12,*) naddl
!!!      elseif(reccor) then
!!!         read (12,*) wlenx, wleny, wlenz
!!!         read (12,*) naddl
!!!      endif
c
   80 format (7(1x,i1))
...
c
!      write(*,*)' '
!      write(*,*)'Computation of Power/Voronoi vertices to begin: ',
!     *          'driver subroutine'
!      write(*,*)'pwrvtx will be called from the main routine and ',
!     *          'vertices will be'
!      write(*,*)'computed.'
!      write(*,*)' '
!      write(*,*)'Please wait ...'
...
!!!      write(*,*)'(Back to the main routine).'
!!!      write(*,*)' '
!!!      write(*,*)'Computation of Power/Voronoi vertices has been ',
!!!     *          'completed.'
!!!      write(*,*)' '
!!!      write(*,*)'Number of points in set whose Power/Voronoi ',
!!!     *          'diagram is'
!!!      write(*,*)'under consideration = ',nw
!!!      write(*,*)' '
!!!      write(*,*)'Number of (bounded) tetrahedra in initial tetrahedron'
!!!      write(*,*)'list, i. e. number of Power/Voronoi vertices = ',nt 
!!!      write(*,*)' '
!!!      write(*,*)'Number of bounded and unbounded tetrahedra in ',
!!!     *          'final'
!!!      write(*,*)'tetrahedron list, i. e. number of Power/Voronoi'
!!!      write(*,*)'vertices and unbounded edges = ',nq
c
c----------------------------------------------------------------------
c
...
c
!      write(*,*)' '
!      write(*,*)'Saving Power/Voronoi vertices and unbounded edges ',
!     *          'information ...'
!      write (13,*) nw, nt, nq
!      write (13,400) (ifl(i),i=1,nq)
!      do 300 i = 1, nq
!         write (13,*) xp(i), yp(i), zp(i)
  300 continue
  400 format (40(1x,i1))
c
c----------------------------------------------------------------------
...
!      write(*,*)' '
!      write(*,*)'Saving final tetrahedralization information ...'
!      write (14,80) ideli, ipnti, iflpi, iarti, irani, ireci, iredi
!      if(.not.delaun) then
!         write (14,*) nw, nt, nq, icfig, iwfig
!      else
!         write (14,*) nw, nt, nq, icfig
!      endif
!      write (14,90) ((icon(i,j), i = 1, 8), j = 1, nq)
!      write (14,95) (is(i), i = 1, nw)
!      if(reccor .and. .not.delaun) then
!         write (14,*) wlenx, wleny, wlenz, wlenw
!         write (14,*) naddl
!      elseif(reccor) then
!         write (14,*) wlenx, wleny, wlenz
!         write (14,*) naddl
!      endif
c
c----------------------------------------------------------------------
...
c
  ...
  520 continue
!      write(*,*)' '
      write(*,*)'All Power/Voronoi vertices marked as too large.'
      go to 900
  530 continue
      do 550 no = 1, nt
...
c
!!!      write(*,*)' '
!!!      write(*,*)'min and max of Power/Voronoi vertices coordinates:'
!!!      write(*,*)' '
!!!      write(*,*)'xmin = ',xpmin
!!!      write(*,*)'xmax = ',xpmax
!!!      write(*,*)'ymin = ',ypmin
...
c
  ...
  800 continue
!!!      write(*,*)' '
!!!      write(*,*)'Number of Power/Voronoi vertices marked as too ',
!!!     *          'large = ',larg
!!!      write(*,*)' '
!!!      write(*,*)'Power/Voronoi distance error = ',derro
!!!      write(*,*)'(This number should be close to zero).'
  900 continue
c
...
c
 ...
 1000 continue
!!!      write(*,*)' '
!!!      write(*,*)'Power/Voronoi unbounded edge error = ',derro
!!!      write(*,*)'(This number should be close to zero).'
c
c----------------------------------------------------------------------
c
c      write(*,*)' '
      ...
      end subroutine pwrvtx_sub


*PWRVTX
c**********************************************************************
c
c     Driver subroutine of Fortran 77 program PWRVTX for computing
c     the Power/Voronoi diagram vertices and unbounded edges for a
c     set of points in 3-dimensional space from a Regular/Delaunay
c     tetrahedralization for the set.
c
c     It is assumed that a Regular/Delaunay tetrahedralization for
c     the set of points has already been computed using existing
c     program regtet/ragtat and that the output tetrahedron list
...
c     artificial ones), i. e. logical variable artfcl was set to
c     .false. during the execution of regtet/ragtat. The output
c     tetrahedron list from regtet/ragtat is then used by this
c     program. It is noted that because of degeneracies more than
c     one tetrahedron may correspond to the same Power/Voronoi vertex.
c     In addition, because of degeneracies, Power/Voronoi cells of
c     points may have facets that are not 2-dimensional, and if
c     weights are present there may be Power cells of points that are
c     not 3-dimensional.
c
c     Arrays xp, yp, zp, icon, ifl will contain output information.
c     For i=1,...,nt, xp(i), yp(i), zp(i) will be the coordinates of
c     the Power/Voronoi vertices.
c     For i=nt+1,...,nq, xp(i), yp(i), zp(i) will be the coordinates
c     of the unbounded edges.
c     For j=1,...,8, i=1,...,nt, icon(j,i) will be the input
c     tetrahedron list modified to include unbounded edges neighbor
c     information.
c     For j=1,...,8, i=nt+1,...,nq, icon(j,i) will be the unbounded
c     edges list which is an extension of the modified input
c     tetrahedron list and is obtained by treating unbounded edges
c     as tetrahedra with one vertex, vertex 0, at infinity.
c     For i=1,...,nt, ifl(i) will equal 1 if the size of Power/Voronoi
c     vertex i is not considered to be too large.
c     If the size of vertex i is considered to be too large ifl(i)
c     will equal 0.
c     For i=nt+1,...,nq, ifl(i) will equal 2.
c
...
c
      ...
      integer icon(8,*), ifl(*), is(*)
      integer nv, nw, nt, nq, nmax, nvmax, naddl, icfig, iwfig
...
c
      ...
      integer isclp(2), isclw(2), isclr(2), mhalf, mfull
c
...
*SETSUP
c
c     This subroutine completes information about tetrahedralization.
c
...
c
      ...
      integer ix2(*), iy2(*), iz2(*), iw2(*)
      double precision xmin, ymin, zmin, wmin, xmax, ymax, zmax
      integer nv, nw, nt, nmax, nvmax, naddl
      double precision dscle, wlenx, wleny, wlenz, wlenw
      integer icsfig, iwsfig, mhalf, mfull
      logical delaun, reccor
      integer isclp(*), isclw(*), isclr(*)
c
      double precision dscli, dfull, dfill, decml
      integer no, irec, irec1, nu, ng, i, naddm, j, icsfi2, irsfig
      integer isgcl, isclu, ibsfig, itsfig
...
c
  165 continue
!!      write(*,*)' '
!!      write(*,*)'min and max of points coordinates:'
!!      write(*,*)' '
!!      write(*,*)'xmin = ',xmin
!!      write(*,*)'xmax = ',xmax
!!      write(*,*)'ymin = ',ymin
...
*VRTCMP
c
c     This subroutine computes Power/Voronoi vertices.
c 
c
...
c
      ...
      double precision xp(*), yp(*), zp(*)
      integer icon(8,*), ifl(*), is(*)
      integer nv, nt, nq, nvmax
      integer mhalf, mfull, nkmax
      logical delaun
      integer isclp(*), isclw(*), isclr(*), ikon(8,1), a, b, c
      parameter(nkmax=30)
      integer io(nkmax), inx(nkmax), iny(nkmax), inz(nkmax)
      double precision r215, dnom, xnum, ynum, znum
      double precision deps, dscle, dscl2, dfull
c
      integer ng, i, j, ifir, isec, ithi, ifou, isgo, iko
      integer isgnx, isgny, isgnz, iknx, ikny, iknz
      integer ist1, ist2, ist3, icur, icar, now
c
c     compute vertices
c
      ng = 0
...
*IDENOM
c
c     Routine for determining denominator used in formulas for
c     computing x-,y-z-coordinates of orthogonal center of
c     tetrahedron with vertices ifir, isec, ithi, ifou.
c
...
*XYZNUM
c
c     Routine for determining numerator used in formulas for
c     computing x-,y-z-coordinates of orthogonal center of
c     tetrahedron with vertices ifir, isec, ithi, ifou.
c
...
c
      ...
      integer inx(*), iny(*), inz(*)
      integer isgnx, isgny, isgnz, iknx, ikny, iknz
      integer ifir, isec, ithi, ifou, mhalf, mfull, nkmax
      integer isclp(*), isclw(*), isclr(*)
      parameter (nkmax = 30)
      integer io(nkmax), iu(nkmax)
      ...
      integer ixf(nkmax), iyf(nkmax), izf(nkmax)
      integer ixf2(nkmax), iyf2(nkmax), izf2(nkmax)
      integer iwf(nkmax), iw2(nkmax), iw3(nkmax), iw4(nkmax)
      logical delaun
      integer iwfuw, iwsew, iwthw, iwfow
      integer ixfuw, iyfuw, izfuw, ixsew, iysew, izsew
      integer ixthw, iythw, izthw, ixfow, iyfow, izfow
      integer iwfu2, iwse2, iwth2, iwfo2
      integer ixfu2, iyfu2, izfu2, ixse2, iyse2, izse2
      integer ixth2, iyth2, izth2, ixfo2, iyfo2, izfo2
      integer isgw2, isgw3, isgw4, ikw2, ikw3, ikw4
...
c
      ...
      subroutine volare_sub(nmax,nvmax,nfmax,nv,xa,ya,za,idumy,nw,nt,nq,
     *                      icon, is, ifl, xp,yp,zp, vol )


c
      integer nmax, nvmax, nfmax
      !parameter (nmax=150000, nvmax= 7*nmax, nfmax = 8*nmax)
      !parameter (nfmax = 8*nmax)
c
      double precision xa(nmax), ya(nmax), za(nmax)
      double precision xp(nvmax), yp(nvmax), zp(nvmax)
      ...
      double precision vol(nmax), area(nfmax), ndst(nfmax)
      !double precision xcor, ycor, zcor, sum,xlixo
      integer nw, nv, nt, nq, nb, icfig
c
      integer idumy, nw2, nt2, nq2 !, i, j
c
c----------------------------------------------------------------------
c
!!!      write(*,*)' '
!!!      write(*,*)'This program is for computing volumes of ',
!!!     *          'Power/Voronoi cells in'
!!!      write(*,*)'the Power/Voronoi diagram of a set of ',
!!!     *          'points in 3-d space, for'
!!!      write(*,*)'computing areas of facets of these cells, ',
!!!     *          'and for computing the'
!!!      write(*,*)'distance between the two points that define ',
!!!     *          'a facet for each facet.'
!!!      write(*,*)' '
!!!      write(*,*)'It is assumed that a Regular/Delaunay ',
...
!!!     *          'and the'
!!!      write(*,*)'unbounded edges of the Power/Voronoi diagram of the ',
!!!     *          'set have'
!!!      write(*,*)'already been computed using existing program pwrvtx ',
!!!     *          'with the'
!!!      write(*,*)'output tetrahedron list from regtet/ragtat as input.'
!!!      write(*,*)' '
!!!      write(*,*)'Besides the list of points in the set, the output ',
!!!     *          'Power/Voronoi'
!!!      write(*,*)'vertex and tetrahedron lists from pwrvtx are then ',
!!!     *          'used as input'
!!!      write(*,*)'by this program. Note that because of degeneracies ',
!!!     *          'there may be'
!!!      write(*,*)'facets that are not 2-dimensional any one of which ',
!!!     *          'will have zero'
!!!      write(*,*)'area if bounded. In addition, if weights are ',
!!!     *          'present, there may'
!!!      write(*,*)'be Power cells that are not 3-dimensional any one ',
!!!     *          'of which will'
!!!      write(*,*)'have zero volume if bounded.'
!!!      write(*,*)' '
!!!      write(*,*)'Arrays ifn, ifc, vol, area, ndst  will contain ',
!!!     *          'output information.'
!!!      write(*,*)'Description of these arrays appears in driver ',
!!!     *          'routine volare.'
!!!      write(*,*)' '
...
!      read (12,*) idumy
!      read (12,*) nw, nt, nq
!      if(nw.gt.nmax .or. nt.ge.nq .or. nq.gt.nvmax) stop 10
...
c
!!      write(*,*)' '
!!      write(*,*)'Reading point/site data ...'
      !nv = 0
  100 continue
!      read (11, *, end = 120) xcor, ycor, zcor
!      nv = nv + 1
!      if (nv .gt. nmax) stop 30
!      xa(nv) = xcor
!      ya(nv) = ycor
...
!!!      read (13,*) nw2, nt2, nq2
      if(nw2.ne.nw .or. nt2.ne.nt .or. nq2.ne.nq) stop 50
...
c
!!      write(*,*)' '
!!      write(*,*)'Computation of volumes, areas, distances to begin: ',
!!     *          'driver subroutine volare'
!!      write(*,*)'will be called from the main routine and volumes, ',
!!     *          'areas, distances'
!!      write(*,*)'will be computed.'
!!      write(*,*)' '
!!      write(*,*)'Please wait ...'
!!      write(*,*)' '
c
...
!!      write(*,*)' '
!!      write(*,*)'(Back to the main routine).'
!!      write(*,*)'Computation of volumes, areas, distances has been ',
!!     *          'completed.'
c
c----------------------------------------------------------------------
c
...
c
!!      write(*,*)' '
!!      write(*,*)'Saving volumes, areas, distances information ...'
...
c     of a set of points in 3-dimensional space, areas of facets of
c     these cells, and the distances obtained by computing for each
c     facet the distance between the two points in the set that
c     define the facet.
c
c     It is assumed that a Regular/Delaunay tetrahedralization for
c     the set has already been computed using existing program
...
c     diagram of the set have already been computed using existing
c     program pwrvtx with the output tetrahedron list from
c     regtet/ragtat as input. The output Power/Voronoi vertex and
c     tetrahedron lists from pwrvtx are then used as input by this
c     program. It is noted that because of degeneracies there may be
c     facets that are not 2-dimensional any one of which will have zero
c     area if bounded. In addition, if weights are present, there may
c     be Power cells that are not 3-dimensional any one of which will
c     have zero volume if bounded.
c
c     Arrays ifn, ifc, vol, area, ndst will contain output information.
...
c     regtet/ragtat; -30.0 if the Power/Voronoi cell of point i is
c     unbounded and has at least one vertex marked as being too large;
c     -20.0 if the cell is unbounded but has no vertices marked as
c     being too large; and -10.0 if the cell is bounded but has at
c     least one vertex marked as being too large.
...
c     For l=1,...,nb, ndst(l) will be the distance between the two
c     neighboring points in the set that define the lth facet of
c     the Power/Voronoi diagram of the set.
c
c     For l=1,...,nb, ifc(l) will equal an integer, 0 < ifc(l) < nw+1.
c     For i=1,...,nw, ifn(i) will equal zero if point i is not active,
c     or was marked as redundant by program regtet/ragtat, or for any
c     point j with i < j, point j is not a Power/Voronoi neighbor of
c     point i. Clearly ifn(nw) is zero since there is no point j for
c     j > nw. Otherwise if l2 equals ifn(i) and l1 equals the largest
c     among 0 and ifn(j), j < i, then -1 < l1 < l2 < nb+1, i < ifc(l)
c     for l = l1+1,...,l2, and point ifc(l) is a Power/Voronoi neighbor
c     of point i for each l, l = l1+1,...,l2. For l = l1+1,...,l2, the
c     facet shared by point i and point ifc(l) is the lth facet of the
c     Power/Voronoi diagram of the set. It is noted that because of
c     degeneracies this facet may be not be 2-dimensional. Also it is
c     noted that nb equals the largest among ifn(i), i = 1,...,nw.
c
...
c     point i is active, not redundant, and for at least one j, i < j,
c     point j is a Power/Voronoi neighbor of point i, two integers l1
c     and l2 are identified so that the points ifc(l), l = l1+1,...,l2,
c     are the Voronoi/Power neighbors of point i for which i < ifc(l).
c     The last inequality guarantees that each facet is identified only
c     once. For such a point i, ifn(i) is not zero and has been set in
c     the program so that the desired value of l2 is actually ifn(i).
c     If ifn(i-1) is not zero then the desired value of l1 is ifn(i-1).
c     But if ifn(i-1) is zero then ifn(i-2) has to be checked in the
c     same way, and so on. This is why l1 is the largest of 0 and
c     ifn(j), j < i. The last inequality means that if i is greater
c     than 1 then l1 will be 0 or the first of ifn(i-1), ifn(i-2),...,
c     ifn(1) that is not zero. If i equals 1 then l1 is definitely 0.
c    
...
c
      ...
      double precision vol(*), area(*), ndst(*)
...
*VACOMP
c
c     This subroutine processes coordinates of Power/Voronoi vertices
c     in preparation for computations of volumes, areas, distances
c
