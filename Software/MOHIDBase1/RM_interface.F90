...
!> The mapping is used to eliminate inactive cells and to use symmetry to decrease the number of cells for which chemistry must 
!> be run.
...
!> If @a e is negative, this method prints an error message corresponding to IRM_RESULT @a e. If @a e is non-negative, no action is 
!> taken.
!> @param id                   The instance @a id returned from @ref RM_Create.
...
!> @retval IRM_RESULT      0 is success, negative is failure (See @ref RM_DecodeError).
!> @see                    @ref RM_OpenFiles, @ref RM_LogMessage, @ref RM_OutputMessage, @ref RM_ScreenMessage, 
...
!> (@ref RM_SpeciesConcentrations2Module). To use these methods the save-species property needs to be turned on 
!> (@ref RM_SetSpeciesSaveOn).
!> If the save-species property is on, RM_FindComponents will generate
!> a list of aqueous species (@ref RM_GetSpeciesCount, @ref RM_GetSpeciesName), their diffusion coefficients at 25 C 
!> (@ref RM_GetSpeciesD25), their charge (@ref RM_GetSpeciesZ).
!> @param id            The instance @a id returned from @ref RM_Create.
...
!> @param id               The instance @a id returned from @ref RM_Create.
!> @retval                 The number of components in the reaction-module component list, negative is failure 
...
!> where @a nxyz is the number of user grid cells and @a ncomps is the result of @ref RM_FindComponents or @ref 
...
!> @param id               The instance @a id returned from @ref RM_Create.
!> @retval                 Number of columns in the current selected-output definition, negative is failure 
!>                         (See @ref RM_DecodeError).
...
INTEGER FUNCTION RM_InitialPhreeqcCell2Module(id, n_user, cell_numbers, n_cell)
    ...
    if (rmf_debug) call Chk_InitialPhreeqcCell2Module(id, cell_numbers, n_cell)
...
SUBROUTINE Chk_InitialPhreeqcCell2Module(id, cell_numbers, n_cell)
    IMPLICIT NONE
    INTEGER, INTENT(in) :: id, n_cell
    INTEGER, INTENT(in), DIMENSION(:) :: cell_numbers
    INTEGER :: errors
    errors = 0
...
!> @retval IRM_RESULT      0 is success, negative is failure (See @ref RM_DecodeError).
!> @see                    @ref RM_OpenFiles, @ref RM_ErrorMessage, @ref RM_OutputMessage, @ref RM_ScreenMessage, 
!>                         @ref RM_WarningMessage.
!> @par Fortran Example:
!> @htmlonly
!> <CODE>
...
!> past a call to RM_MpiWorker. The workers perform developer-defined calculations, and then RM_MpiWorker is called again to respond
